...
(function( $ ) {
	...
	wp.customize.bind( 'preview-ready', function() {
		...
		wp.customize.preview.bind( 'section-highlight', function( data ) {
			...
			if ( ! $( 'body' ).hasClass( 'tophica-front-page' ) ) {
				return;
			}
	...
	wp.customize( 'header_textcolor', function( value ) {
		value.bind( function( to ) {
			if ( 'blank' === to ) {
				...
				if ( ! to.length ) {
					$( '#tophica-custom-header-styles' ).remove();
				}
				$( '.site-title, .site-description' ).css({
					clip: 'auto',
					position: 'relative'
	...
	wp.customize( 'page_layout', function( value ) {
		value.bind( function( to ) {
			if ( 'layout-2cr' === to ) {
				$( 'body' ).addClass( 'layout-2cr' ).removeClass( 'layout-2cl' );
			} else {
				$( 'body' ).removeClass( 'layout-2cr' ).addClass( 'layout-2cl' );
			}
		} );
	} );
